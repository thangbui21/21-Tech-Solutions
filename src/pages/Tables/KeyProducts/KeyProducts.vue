...
<div>  <b-row>
    <b-col>
      <!--Đồ uống lấy trong product.type-->
      <Widget
        title="<h5><span class='fw-semi-bold'>Đồ uống</span></h5>"
        customHeader
        settings
        close
      >
        <!--Cần nhiều thông tin hơn ở 1 bảng to như này-->
        <div class="table-resposive">
          <table class="table">
            ...
            <tbody v-for="(row, index) in keyproduct" v-bind:key="index">
              <tr>
                <td>{{row.id}}</td>
...
export default {
  data() {
    ...
    UserService
      .get("/api/products")
      .then((response) => (this.keyproduct = response.data));
  },
  name: "Key Products",
  methods: {},
  computed: {},
